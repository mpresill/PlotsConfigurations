...
###### <<<<<< this can be commented after the first plot
DATACARD_FIT=/eos/user/m/mpresill/CMS/VBS/VBS_ZV/Datacards/YearsCombination_13Jan2023/combined_card_all_comb       ### this is for fitting the three years combined
echo "${DATACARD_FIT}"
...
PostFitShapesFromWorkspace \
    ...
    --postfit --sampling \
    -f /eos/user/m/mpresill/CMS/VBS/VBS_ZV/fit/YearsCombination_13Jan2023/fitDiagnosticsTest.root:fit_s \
    --total-shapes                     
...
rm -r plot_combined/*
    ...
    # we use edited plotter, to allow bin width normalization
../scripts/mkPostFitCombinedPlot2.py \
...
cp -r *combined.py /eos/user/m/mpresill/www/VBS/postfit/PlotsVBS_ZV_${date}/${Category}/${CUT2}/${PLOTVAR}/.
        ### backup to current AN folder for overleaf
cp -r plot_combined/*png /eos/user/m/mpresill/www/VBS/AN_v8_plots/${Category}_${CUT2}_${PLOTVAR}_postfit2016.png
echo "/eos/user/m/mpresill/www/VBS/AN_v8_plots/${Category}_${CUT2}_${PLOTVAR}_postfit2016.png"
####################################################################
...
cp -r *combined.py /eos/user/m/mpresill/www/VBS/prefit/PlotsVBS_ZV_${date}/${Category}/${CUT2}/${PLOTVAR}/.
        ### backup to current AN folder for overleaf
cp -r plot_combined/*png /eos/user/m/mpresill/CMS/VBS/VBS_ZV/AN_v5/${Category}_${CUT2}_${PLOTVAR}_prefit2016.png
cp -r plot_combined/*png /eos/user/m/mpresill/www/VBS/AN_v8_plots/${Category}_${CUT2}_${PLOTVAR}_prefit2016.png
        ### backup to current AN folder for overleaf
cp -r plot_combined/*png /eos/user/m/mpresill/www/VBS/AN_v8_plots/${Category}_${CUT2}_${PLOTVAR}_prefit2016.png
