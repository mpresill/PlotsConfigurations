...
mc_common = ["DY", "top", "WJets", "WW", "ggWW", "Vg", "VgS",  "VVV", "VBF-V","ZZlep"] #"tZq_ll","VZ",
...
nuisances['PS_ISR']  = {
    ...
    'kind': 'weight',
    'type': 'shape',
    'samples': dict((skey, ['PSWeight[2]', 'PSWeight[0]']) for skey in mc if skey not in ['Vg','VgS','WJets','WW']), #PSWeights are buggy for some samples, we add them back by hand below
    #'AsLnN': '1',
}
...
nuisances['PS_FSR']  = {
    ...
    'kind': 'weight',
    'type': 'shape',
    'samples': dict((skey, ['PSWeight[3]', 'PSWeight[1]']) for skey in mc if skey not in ['Vg','VgS','WJets','WW']), #PSWeights are buggy for some samples, we add them back by hand below
    #'AsLnN': '1',
}
#nuisances['PS_ISR']  = {
#    'name': 'PS_ISR',
#    'kind': 'weight',
#    'type': 'shape',
#    'samples': dict((skey, ['PSWeight[2]', 'PSWeight[0]']) for skey in mc if skey in ['sm_dipole']), #PSWeights are buggy for some samples, we add them back by hand below
#    'cuts'  : [
#                'Boosted_SR_bVeto',
#                'Boosted_SR_bTag',
#                'Boosted_DYcr_bVeto',
...
#    'kind': 'weight',
#    'type': 'shape',
#    'samples': dict((skey, ['PSWeight[3]', 'PSWeight[1]']) for skey in mc if skey in ['sm_dipole']), #PSWeights are buggy for some samples, we add them back by hand below
#    'cuts'  : [
#                'Boosted_SR_bVeto',
#                'Boosted_SR_bTag',
#                'Boosted_DYcr_bVeto',
...
for sample in mc_common :
    if sample in ["ggWW","WW","DY"]: continue   #this sample apparently doesn't have LHE weights, but it's real minor for us
...
#        }
    ...
    # UNCOMMENT FOR EFT analysis NOt added for the moment to stay minimal
...
#}
    ...
    # pdf uncertainty on signal acceptance  NOt added for the moment to stay minimal
...
"""nuisances['Topnorm_boosted_bVeto']  = {
               'name'  : 'Topnorm_boosted_bVeto_2017',
               'samples'  : {
                   'top' : '1.00',
                   },
               'type'  : 'rateParam',
...
nuisances['Topnorm_resolved_bTag']  = {
               'name'  : 'Topnorm_resolved_bTag_2017',
               'samples'  : {
                   'top' : '1.00',
                   },
               'type'  : 'rateParam',
