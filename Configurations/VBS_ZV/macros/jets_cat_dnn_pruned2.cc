...
// --- functions Helper
float deltaEta2(float eta1, float eta2) {
...
class jets_cat_dnn2 : public multidraw::TTreeFunction {
public:
  jets_cat_dnn2( char const* _type, const char* year, const char* model_dir, const char* model_dir_pruned, bool verbose);
  jets_cat_dnn2( unsigned type, const char* year, const char* model_dir, const char* model_dir_pruned, bool verbose);

  char const* getName() const override { return "jets_cat_dnn2"; }
  TTreeFunction* clone() const override { return new jets_cat_dnn2(returnVar_,year_.c_str(), model_dir_.c_str(),model_dir_pruned_.c_str(), verbose ); }
...
std::tuple<UInt_t, UInt_t, ULong64_t> jets_cat_dnn2::currentEvent{};

UIntValueReader* jets_cat_dnn2::nFatJet{}; 
FloatArrayReader* jets_cat_dnn2::FatJet_pt{};
FloatArrayReader* jets_cat_dnn2::FatJet_eta{};
FloatArrayReader* jets_cat_dnn2::FatJet_phi{};
FloatArrayReader* jets_cat_dnn2::FatJet_mass{};
FloatArrayReader* jets_cat_dnn2::Jet_qgl{};


IntArrayReader*   jets_cat_dnn2::CleanJet_jetId{};
IntArrayReader*   jets_cat_dnn2::CleanFatJet_jetId{};
IntArrayReader*   jets_cat_dnn2::CleanJetNotFat_jetId{};
FloatArrayReader* jets_cat_dnn2::Jet_mass{};
UIntValueReader*  jets_cat_dnn2::nCleanJetNotFat; 
FloatArrayReader* jets_cat_dnn2::CleanJet_pt{};
FloatArrayReader* jets_cat_dnn2::CleanJet_eta{};
FloatArrayReader* jets_cat_dnn2::CleanJet_phi{};
FloatArrayReader* jets_cat_dnn2::Lepton_pt{};
FloatArrayReader* jets_cat_dnn2::Lepton_eta{};
FloatArrayReader* jets_cat_dnn2::Lepton_phi{};
UIntValueReader* jets_cat_dnn2::nLepton;
FloatValueReader*  jets_cat_dnn2::mll{};
FloatArrayReader* jets_cat_dnn2::Jet_btagDeepB{};

string jets_cat_dnn2::year_{};
//string jets_cat_dnn2::model_dir_{};
//bool jets_cat_dnn2::verbose{};

std::array<double, jets_cat_dnn2::nVarTypes> jets_cat_dnn2::returnValues{};


// function Helper ---


jets_cat_dnn2::jets_cat_dnn2( char const* _type, const char* year, const char* model_dir, const char* model_dir_pruned, bool verbose):
   TTreeFunction(), model_dir_(model_dir), model_dir_pruned_(model_dir_pruned), verbose(verbose){
    ...
    jets_cat_dnn2::year_ = year;
    // jets_cat_dnn2::model_dir_ = model_dir;
    // jets_cat_dnn2::verbose = verbose;
}

jets_cat_dnn2::jets_cat_dnn2( unsigned type, const char* year, const char* model_dir, const char* model_dir_pruned,bool verbose):
...
double
jets_cat_dnn2::evaluate(unsigned)
{
  ...
  if (category ==0 ){
	...
	//cout<< "jetscat2 full" <<endl;
     //          cout << "boosted breq full" << endl;
...
//Resolved
  ...
  if (category ==0 ){
          //cout <<"event "<<*event->Get()<<  " : Boosted  "<<returnValues[vbs_category]<<" vbs1 : " << vbs_jet1 << " vbs2: "<< vbs_jet2 <<  endl;
//
//cout << "jetscat2 pruned "<< endl;
         //cout << "boosted breq pruned" << endl;
      float Zlep_1 = ((Lepton_eta->At(0)) - 0.5 * ((CleanJet_eta->At(vbs_jet1)) + (CleanJet_eta->At(vbs_jet2)))) / detajj;
      float Zlep_2 = ((Lepton_eta->At(1)) - 0.5 * ((CleanJet_eta->At(vbs_jet1)) + (CleanJet_eta->At(vbs_jet2)))) / detajj;
      float Zvjet = ((FatJet_eta->At(0)) - 0.5 * ((CleanJet_eta->At(vbs_jet1)) + (CleanJet_eta->At(vbs_jet2)))) / detajj;
      std::vector<float> input{};
...
//Resolved
    ...
    //cout <<"event "<<*event->Get()<<  " : Resolved  "<<" vbs1 : " << vbs_jet1 << " vbs2: "<< vbs_jet2 <<" v1 : " << v_jet1 << " v2: "<< v_jet2 <<  endl;
         //cout << "resolved breq pruned" << endl;
      float Zlep_1 = ((Lepton_eta->At(0)) - 0.5 * ((CleanJet_eta->At(vbs_jet1)) + (CleanJet_eta->At(vbs_jet2)))) / detajj;
      float Zlep_2 = ((Lepton_eta->At(1)) - 0.5 * ((CleanJet_eta->At(vbs_jet1)) + (CleanJet_eta->At(vbs_jet2)))) / detajj;
...
void
jets_cat_dnn2::bindTree_(multidraw::FunctionLibrary& _library)
{   
...
/*static*/
void
jets_cat_dnn2::setValues(UInt_t _run, UInt_t _luminosityBlock, ULong64_t _event)
{
  ...
  //btag cut values
  //cout << jets_cat_dnn2::year_ <<endl;
  if (jets_cat_dnn2::year_ == "2018") btag_cut=0.1241;
  else if (jets_cat_dnn2::year_ == "2017") btag_cut = 0.1522;
  else if (jets_cat_dnn2::year_ == "2016") btag_cut = 0.2217;
  //cout << btag_cut <<endl;
  ...
  if (njet>=2){
        ...
        for (unsigned int ijet=0 ; ijet<(njet-1) ; ijet++){
            for (unsigned int jjet= ijet+1 ; jjet<njet ; jjet++){
                ...
                if( Mjj_tmp >= Mjj_max ){
                    Mjj_max=Mjj_tmp;
                    detajj_mjj_max=deltaEta2((vectors.at(ijet)).Eta(),(vectors.at(jjet)).Eta());
                    dphijj_mjj_max=deltaPhi2((vectors.at(ijet)).Phi(),(vectors.at(jjet)).Phi());
                    // Index in vectors
                    VBS_jets[0]= ijet;
                    VBS_jets[1]= jjet;
                }
