...
// --- functions Helper
float deltaEtaFJ(float eta1, float eta2) {
  return std::abs(eta1 - eta2);
}

float deltaPhiFJ(float phi1, float phi2){
...
class jets_cat_dnn_FJ : public multidraw::TTreeFunction {
public:
  jets_cat_dnn_FJ( char const* _type, const char* year, const char* model_dir, const char* model_dir_pruned, bool verbose);
  jets_cat_dnn_FJ( unsigned type, const char* year, const char* model_dir, const char* model_dir_pruned, bool verbose);

  char const* getName() const override { return "jets_cat_dnn_FJ"; }
  TTreeFunction* clone() const override { return new jets_cat_dnn_FJ(returnVar_,year_.c_str(), model_dir_.c_str(),model_dir_pruned_.c_str(), verbose ); }
...
std::tuple<UInt_t, UInt_t, ULong64_t> jets_cat_dnn_FJ::currentEvent{};

UIntValueReader* jets_cat_dnn_FJ::nFatJet{}; 
FloatArrayReader* jets_cat_dnn_FJ::FatJet_pt{};
FloatArrayReader* jets_cat_dnn_FJ::FatJet_eta{};
FloatArrayReader* jets_cat_dnn_FJ::FatJet_phi{};
FloatArrayReader* jets_cat_dnn_FJ::FatJet_mass{};
FloatArrayReader* jets_cat_dnn_FJ::Jet_qgl{};


IntArrayReader*   jets_cat_dnn_FJ::CleanJet_jetId{};
IntArrayReader*   jets_cat_dnn_FJ::CleanFatJet_jetId{};
IntArrayReader*   jets_cat_dnn_FJ::CleanJetNotFat_jetId{};
FloatArrayReader* jets_cat_dnn_FJ::Jet_mass{};
UIntValueReader*  jets_cat_dnn_FJ::nCleanJetNotFat; 
FloatArrayReader* jets_cat_dnn_FJ::CleanJet_pt{};
FloatArrayReader* jets_cat_dnn_FJ::CleanJet_eta{};
FloatArrayReader* jets_cat_dnn_FJ::CleanJet_phi{};
FloatArrayReader* jets_cat_dnn_FJ::Lepton_pt{};
FloatArrayReader* jets_cat_dnn_FJ::Lepton_eta{};
FloatArrayReader* jets_cat_dnn_FJ::Lepton_phi{};
UIntValueReader* jets_cat_dnn_FJ::nLepton;
FloatValueReader*  jets_cat_dnn_FJ::mll{};
FloatArrayReader* jets_cat_dnn_FJ::Jet_btagDeepB{};

string jets_cat_dnn_FJ::year_{};
//string jets_cat_dnn_FJ::model_dir_{};
//bool jets_cat_dnn_FJ::verbose{};

std::array<double, jets_cat_dnn_FJ::nVarTypes> jets_cat_dnn_FJ::returnValues{};


// function Helper ---


jets_cat_dnn_FJ::jets_cat_dnn_FJ( char const* _type, const char* year, const char* model_dir, const char* model_dir_pruned, bool verbose):
   TTreeFunction(), model_dir_(model_dir), model_dir_pruned_(model_dir_pruned), verbose(verbose){
    ...
    jets_cat_dnn_FJ::year_ = year;
    // jets_cat_dnn_FJ::model_dir_ = model_dir;
    // jets_cat_dnn_FJ::verbose = verbose;
}

jets_cat_dnn_FJ::jets_cat_dnn_FJ( unsigned type, const char* year, const char* model_dir, const char* model_dir_pruned,bool verbose):
...
double
jets_cat_dnn_FJ::evaluate(unsigned)
{
...
//Resolved
  ...
  if (category ==0 ){
          //cout <<"event "<<*event->Get()<<  " : Boosted  "<<returnValues[vbs_category]<<" vbs1 : " << vbs_jet1 << " vbs2: "<< vbs_jet2 <<  endl;
        // cout << "boosted bveto pruned" << endl;
      float Zlep_1 = ((Lepton_eta->At(0)) - 0.5 * ((CleanJet_eta->At(vbs_jet1)) + (CleanJet_eta->At(vbs_jet2)))) / detajj;
      float Zlep_2 = ((Lepton_eta->At(1)) - 0.5 * ((CleanJet_eta->At(vbs_jet1)) + (CleanJet_eta->At(vbs_jet2)))) / detajj;
      float Zvjet = ((FatJet_eta->At(0)) - 0.5 * ((CleanJet_eta->At(vbs_jet1)) + (CleanJet_eta->At(vbs_jet2)))) / detajj;
      std::vector<float> input{};
...
//Resolved
    ...
    //cout <<"event "<<*event->Get()<<  " : Resolved  "<<" vbs1 : " << vbs_jet1 << " vbs2: "<< vbs_jet2 <<" v1 : " << v_jet1 << " v2: "<< v_jet2 <<  endl;
      //   cout << "resolved bveto pruned" << endl;
      float Zlep_1 = ((Lepton_eta->At(0)) - 0.5 * ((CleanJet_eta->At(vbs_jet1)) + (CleanJet_eta->At(vbs_jet2)))) / detajj;
      float Zlep_2 = ((Lepton_eta->At(1)) - 0.5 * ((CleanJet_eta->At(vbs_jet1)) + (CleanJet_eta->At(vbs_jet2)))) / detajj;
...
void
jets_cat_dnn_FJ::bindTree_(multidraw::FunctionLibrary& _library)
{   
    ...
    _library.bindBranch(FatJet_pt, "FatJet_pt_nom");          //CleanFatJet_pt updated to FatJet_pt_nom, then reference to CleanFatJet_jetIdx[0] is given.
    _library.bindBranch(FatJet_eta, "CleanFatJet_eta");
    _library.bindBranch(FatJet_phi, "CleanFatJet_phi");
    _library.bindBranch(FatJet_mass, "FatJet_msoftdrop_nom"); //CleanFatJet_mass updated to FatJet_msoftdrop_nom, then reference to CleanFatJet_jetIdx[0] is given.
...
/*static*/
void
jets_cat_dnn_FJ::setValues(UInt_t _run, UInt_t _luminosityBlock, ULong64_t _event)
{
  ...
  //btag cut values
  //cout << jets_cat_dnn_FJ::year_ <<endl;
  if (jets_cat_dnn_FJ::year_ == "2018") btag_cut=0.1241;
  else if (jets_cat_dnn_FJ::year_ == "2017") btag_cut = 0.1522;
  else if (jets_cat_dnn_FJ::year_ == "2016") btag_cut = 0.2217;
  //cout << btag_cut <<endl;
  ...
  if (njet>=2){
        ...
        for (unsigned int ijet=0 ; ijet<(njet-1) ; ijet++){
            for (unsigned int jjet= ijet+1 ; jjet<njet ; jjet++){
                ...
                if( Mjj_tmp >= Mjj_max ){
                    Mjj_max=Mjj_tmp;
                    detajj_mjj_max=deltaEtaFJ((vectors.at(ijet)).Eta(),(vectors.at(jjet)).Eta());
                    dphijj_mjj_max=deltaPhiFJ((vectors.at(ijet)).Phi(),(vectors.at(jjet)).Phi());
                    // Index in vectors
                    VBS_jets[0]= ijet;
                    VBS_jets[1]= jjet;
                }
        ...
        if (nFJ >= 1){
            ...
            _Vpt = FatJet_pt->At(CleanFatJet_jetId->At(0));                  //updated the reference to cleaned fat jet class
