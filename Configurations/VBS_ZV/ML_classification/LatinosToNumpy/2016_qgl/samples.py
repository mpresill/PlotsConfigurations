...
addSampleWeight(samples,'VBS_ZV','WmTo2J_ZTo2L_dipoleRecoil', '(Sum(AbsVec(GenPart_pdgId) == 6) ==0)')
addSampleWeight(samples,'VBS_ZV','WpTo2J_ZTo2L_dipoleRecoil', '(Sum(AbsVec(GenPart_pdgId) == 6) ==0)')
"""
samples['tZq'] = {
...
for _, sd in DataRun:
  for pd in DataSets:
    iles = nanoGetSampleFiles(dataDirectory, pd + '_' + sd)
