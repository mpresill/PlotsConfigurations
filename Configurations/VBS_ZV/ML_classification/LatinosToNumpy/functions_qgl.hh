...
string filename = "/afs/cern.ch/work/m/mpresill/Latino/CMSSW_10_6_4/src/PlotsConfigurations/Configurations/patches/PUID_81XTraining_EffSFandUncties.root";
string year = "2018";
string wp_ = "L";

//inititalise morph functions
std::map<std::string, TGraph*> morphing_functions{};
...
void morph_init(string morph_file, string morph_loweta_gluon_pt0, string morph_loweta_gluon_pt1, string morph_higheta_gluon_pt0, string morph_higheta_gluon_pt1, string morph_loweta_quark_pt0, string morph_loweta_quark_pt1, string morph_higheta_quark_pt0, string morph_higheta_quark_pt1) {
	TFile *rfile =TFile::Open(morph_file.c_str());
    	morphing_functions["gluon_loweta_pt0"] = (TGraph*) rfile->Get(morph_loweta_gluon_pt0.c_str());
    	morphing_functions["gluon_loweta_pt1"] = (TGraph*) rfile->Get(morph_loweta_gluon_pt1.c_str());
    	morphing_functions["gluon_higheta_pt0"] = (TGraph*) rfile->Get(morph_higheta_gluon_pt0.c_str());
    	morphing_functions["gluon_higheta_pt1"] = (TGraph*) rfile->Get(morph_higheta_gluon_pt1.c_str());
    	morphing_functions["quark_loweta_pt0"] = (TGraph*) rfile->Get(morph_loweta_quark_pt0.c_str());
    	morphing_functions["quark_loweta_pt1"] = (TGraph*) rfile->Get(morph_loweta_quark_pt1.c_str());
    	morphing_functions["quark_higheta_pt0"] = (TGraph*) rfile->Get(morph_higheta_quark_pt0.c_str());
    	morphing_functions["quark_higheta_pt1"] = (TGraph*) rfile->Get(morph_higheta_quark_pt1.c_str());
    	delete rfile;
	morph_ini=false;
...
*/
float getMorphedGluon(float x, float eta, float pt){
  //cout << "gluon x,eta, pt = " << x << eta << pt <<endl;
  if (x<= 0.) return x;
  if (x>= 1.) return x;
  float y = x;
  if (abs(eta)<3 && pt < 75  && do_morph_gluon_loweta_pt0) {
//      cout <<"gluon low eta lowpt "<<endl;
          y =  morphing_functions["gluon_loweta_pt0"]->Eval(x);}
  if (abs(eta)<3 && pt >= 75  && do_morph_gluon_loweta_pt1)
          y =  morphing_functions["gluon_loweta_pt1"]->Eval(x);
  if (abs(eta)>=3 && pt < 75  && do_morph_gluon_higheta_pt0)
          y =  morphing_functions["gluon_higheta_pt0"]->Eval(x);
  if (abs(eta)>=3 && pt >= 75  && do_morph_gluon_higheta_pt1)
          y =  morphing_functions["gluon_higheta_pt1"]->Eval(x);
  if (y<0) return 0.;
  if (y>1.) return 1.;
  return y;
}

float getMorphedQuark(float x, float eta, float pt){
  //cout << "quark x,eta, pt = " << x << eta << pt <<endl;
  if (x<= 0.) return x;
  if (x>= 1.) return x;
  float y = x ;
  if (abs(eta)<3 && pt < 75  && do_morph_quark_loweta_pt0)
          y =  morphing_functions["quark_loweta_pt0"]->Eval(x);
  if (abs(eta)<3 && pt >= 75  && do_morph_quark_loweta_pt1){
//      cout << "quark low eta high pt" <<endl;
          y =  morphing_functions["quark_loweta_pt1"]->Eval(x);
//      cout << "y =" << y <<endl;
        }
  if (abs(eta)>=3 && pt < 75  && do_morph_quark_higheta_pt0){
//      cout << "quark high eta low pt" <<endl;
          y =  morphing_functions["quark_higheta_pt0"]->Eval(x);
        //cout << "y="<< y<<endl;
        }
  if (abs(eta)>=3 && pt >= 75  && do_morph_quark_higheta_pt1)
          y =  morphing_functions["quark_higheta_pt1"]->Eval(x);
  if (y<0) return 0.;
  if (y>1.) return 1.;
  return y;
}
...
//add morphing inputs 
RVec<float> jets_cat(int nCleanJetNotFat,int  nCleanFatJet,RVec<float> & CleanJet_pt, RVec<int> & CleanJetNotFat_jetId, RVec<float> & CleanJet_phi, RVec<float> & CleanJet_eta, RVec<float> & Jet_mass, RVec<int> & CleanJet_jetId, RVec<float> & CleanFatJet_mass, int nLepton, RVec<float> & Lepton_eta, RVec<float> & CleanFatJet_eta, RVec<float> & Jet_qgl, RVec<int> & CleanFatJet_jetid, RVec<float> & Jet_btagDeepB, RVec<float> & Lepton_pt, RVec<float> & Lepton_phi, int year,RVec<int> & Jet_partonFlavour,  string morph_file, string do_morph,string morph_loweta_gluon_pt0, string morph_loweta_gluon_pt1,string morph_higheta_gluon_pt0, string morph_higheta_gluon_pt1,string morph_loweta_quark_pt0, string morph_loweta_quark_pt1, string morph_higheta_quark_pt0, string morph_higheta_quark_pt1 ){
...
//initialise morphing functions
if (morph_ini) {
	morph_init(morph_file,morph_loweta_gluon_pt0,morph_loweta_gluon_pt1,  morph_higheta_gluon_pt0,  morph_higheta_gluon_pt1,  morph_loweta_quark_pt0,  morph_loweta_quark_pt1,  morph_higheta_quark_pt0,  morph_higheta_quark_pt1);	
}
int do_morph_flags = std::stoi(do_morph.c_str(),0,2);
do_morph_gluon_loweta_pt0 = do_morph_flags & 1;
do_morph_gluon_loweta_pt1 = do_morph_flags >>1 & 1;
do_morph_gluon_higheta_pt0 = do_morph_flags >> 2 & 1;
do_morph_gluon_higheta_pt1 = do_morph_flags >> 3 & 1;
do_morph_quark_loweta_pt0 = do_morph_flags >> 4 & 1;
do_morph_quark_loweta_pt1 = do_morph_flags >> 5 & 1;
do_morph_quark_higheta_pt0 = do_morph_flags >> 6 & 1;
do_morph_quark_higheta_pt1 = do_morph_flags >> 7 & 1;
...
RVec<float> cat{Mjj_max,detajj_mjj_max,dphijj_mjj_max,Vjet_mass_tmp, Zepp_ll, Zlep_1, Zlep_2, 999,999,999,999,999,999,999,999, category, Njet, Zfatjet, 999, 999,999,999,999,999, 999, 999,999,999,999};
cat[23] = _nbtag;
cat[24] = _Zleppt;
if (vbs_jet_0 != 999){
     ...
     int vbs_jet_0_pf = Jet_partonFlavour.at(CleanJet_jetId.at(vbs_jet_0));
     cat[25] = vbs_jet_0_pf==21 ? getMorphedGluon(Jet_qgl.at(CleanJet_jetId.at(vbs_jet_0)), CleanJet_eta.at(vbs_jet_0), CleanJet_pt.at(vbs_jet_0)) : getMorphedQuark(Jet_qgl.at(CleanJet_jetId.at(vbs_jet_0)), CleanJet_eta.at(vbs_jet_0), CleanJet_pt.at(vbs_jet_0)); 
}
if (vbs_jet_1 != 999){
     cat[8]= CleanJet_pt.at(vbs_jet_1);
     cat[10]= CleanJet_eta.at(vbs_jet_1);
     cat[19] = Jet_qgl.at(CleanJet_jetId.at(vbs_jet_1));
     int vbs_jet_1_pf = Jet_partonFlavour.at(CleanJet_jetId.at(vbs_jet_1));
     cat[26] = vbs_jet_1_pf==21 ? getMorphedGluon(Jet_qgl.at(CleanJet_jetId.at(vbs_jet_1)), CleanJet_eta.at(vbs_jet_1), CleanJet_pt.at(vbs_jet_1)) : getMorphedQuark(Jet_qgl.at(CleanJet_jetId.at(vbs_jet_1)), CleanJet_eta.at(vbs_jet_1), CleanJet_pt.at(vbs_jet_1));
}
...
if (category==1){
    if (v_jet_0 != 999){
        ...
        int v_jet_0_pf = Jet_partonFlavour.at(CleanJet_jetId.at(v_jet_0));
        cat[27] = v_jet_0_pf==21 ? getMorphedGluon(Jet_qgl.at(CleanJet_jetId.at(v_jet_0)), CleanJet_eta.at(v_jet_0), CleanJet_pt.at(v_jet_0)) : getMorphedQuark(Jet_qgl.at(CleanJet_jetId.at(v_jet_0)), CleanJet_eta.at(v_jet_0), CleanJet_pt.at(v_jet_0));
    }
    if (v_jet_1 != 999){
     cat[12]=CleanJet_pt.at(v_jet_1);
     cat[14]=CleanJet_eta.at(v_jet_1);
     cat[21] = Jet_qgl.at(CleanJet_jetId.at(v_jet_1));
     int v_jet_1_pf = Jet_partonFlavour.at(CleanJet_jetId.at(v_jet_1));
     cat[28] = v_jet_1_pf==21 ? getMorphedGluon(Jet_qgl.at(CleanJet_jetId.at(v_jet_1)), CleanJet_eta.at(v_jet_1), CleanJet_pt.at(v_jet_1)) : getMorphedQuark(Jet_qgl.at(CleanJet_jetId.at(v_jet_1)), CleanJet_eta.at(v_jet_1), CleanJet_pt.at(v_jet_1));
    }
}
if (category==0) {
