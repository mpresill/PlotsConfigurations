...
# balance signal and backgrounds
def normalize(data):
...
def groupplot_var(dir,df,var,xmin=0, xmax=1000,nbins=50, weight='std'):
    ...
    groups=['DY','top','other','signal', 'fake']
...
def prep_data_multi(years, version,cut, vers,  dir):
    ...
    for year in years:
        ...
        for sample in samples_name:
            if 'DY' in sample: samples_group.append('DY')
            elif 'DATA' in sample: samples_group.append('DATA')
            elif 'top' in sample: samples_group.append('top')
            elif 'dipole' in sample : samples_group.append('dipoleRecoil')
            elif 'VBS_ZV' in sample: samples_group.append('signal')
            elif 'Fake' in sample: samples_group.append('fake')
            else : samples_group.append('other')
        ...
        for i, name in enumerate(files):
            ...
            if '2018' in year:
                #print('lumi = 59.74')
                samples_df[i]['weight_'][samples_df[i]['group'] !='fake'] *= 59.74
            elif '2017' in year:
                #print('lumi = 41.53')
                samples_df[i]['weight_'][samples_df[i]['group'] !='fake'] *= 41.53
            elif '2016' in year:
                #print('lumi = 35.867')
                samples_df[i]['weight_'][samples_df[i]['group'] !='fake'] *= 35.867
            samples_df[i].dropna(inplace=True)
            df=df.append(samples_df[i], sort=False)
    #print('postproc')
    #print(len(df))
    df['signal'] = df['group'] == 'signal'
...
def prep_data_select(years, version,cut, vers,  dir, select=False):
