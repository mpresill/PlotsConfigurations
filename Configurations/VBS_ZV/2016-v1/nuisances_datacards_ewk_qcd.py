...
for js in jes_systs:
  ...
  nuisances[js] = {
      ...
      'mapUp': js+'up',
      'mapDown': js+'do',
      'samples': dict((skey, ['1', '1']) for skey in mc),  # if skey not in ['ggWW'] ### if skey not in ['DY'] ###CHECK IF THIS IS STILL TRUE: Do we have all the DY samples shapes UP/DOWN for this available?
      'folderUp': folderup,
      'folderDown': folderdo,
     #'group': 'AK4jet',
     # 'AsLnN': '1' 
...
### uncommenting the following lines at the moment of datacard making
for sample in mc_eos :       #### VBS_VV_QCD has bugged QCDscale weights for 2016, it is extracted as normalization factors from 2018
    nuisances['QCDscale_'+sample] = {
            'name'  : 'QCDscale_'+sample,
            'kind': 'weight_envelope',
            'type'  : 'shape',
...
nuisances['pdf_weight'] = { ### UPDATED FOR 2016 SAMPLES: NEED TO USE RMS HERE!!!
    ...
    'kind'  : 'weight_rms',    
    'type'  : 'shape',
    'samples' :  { s: ["LHEPdfWeight[%d]" %i for i in range(100)] for s in ["Vg", "VgS","VBF-V","VBS_WV_QCD"] }, #-> here we reomve bkgs measured in CR (DY and top, and WW for which weights are bugged), as well as BSM signals (PHDF4LHC prescription for BSM measurement)
    #'group' : 'theory',
    'AsLnN':  '1'
}
