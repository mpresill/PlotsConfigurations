...
class ShapeFactory:
    ...
    def makeNominals(self, inputDir, outputDir, variables, cuts, samples, nuisances, supercut, number=99999, firstEvent=0, nevents=-1):
	  ...
	  #Lourdes
	  print("sampleName", sample, sampleName)
          ...
          for nuisanceName, nuisance in nuisances.iteritems():
            ...
            if nkind.startswith('tree'):
              ...
              for var in variations:
                ...
                #USING TWO ALIASES AS A SHAPE NUISANCE PARAMETER (NEW lines start here)
		#_____________________________________________________________________________
                elif 'auxname' in nuisance:
                  #print('Use of auxname detected')
                  ...
                  for bname in bmap['branches']:
                    #print('Replacing bname', bname, ' by ', nuisance['auxname'] + bmap['auxsuffix'])
                    ndrawer.replaceBranch(bname, nuisance['auxname'] + bmap['auxsuffix'])
		#______________________________________________________________________________END new lines
                  ...
                  print("ndrawer", nuisanceName, var, sampleName)
		  ...
		  friendsDir=(nuisance['folder' + var], friendAlias)
                  prefix = friendAlias + '.'
                else:
                  ndrawer = nuisanceDrawers[nuisanceName][var] = self._connectInputs(sampleName, sample['name'], inputDir, skipMissingFiles=False)
                  prefix = ''
    ...
    def _addUncertaintyOn0bincontent(histogram_to_be_fixed):
      ...
      for i in range(1, histogram_to_be_fixed.GetXaxis().GetNbins()+1):
