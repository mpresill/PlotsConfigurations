...
RVec<float> jets_cat(int nCleanJetNotFat,int  nCleanFatJet,RVec<float> & CleanJet_pt, RVec<int> & CleanJetNotFat_jetId, RVec<float> & CleanJet_phi, RVec<float> & CleanJet_eta, RVec<float> & Jet_mass, RVec<int> & CleanJet_jetId, RVec<float> & CleanFatJet_mass, int nLepton, RVec<float> & Lepton_eta, RVec<float> & CleanFatJet_eta, RVec<float> & Jet_qgl, RVec<int> & CleanFatJet_jetid, RVec<float> & Jet_btagDeepB, int year, RVec<int>& Jet_partonFlavour){
...
RVec<float> cat{Mjj_max,detajj_mjj_max,dphijj_mjj_max,Vjet_mass_tmp, Zepp_ll, Zlep_1, Zlep_2, 999,999,999,999,999,999,999,999, category, Njet, Zfatjet, 999, 999,999,999,999,999, 999,999,999,99};
cat[23] = _nbtag;
if (vbs_jet_0 != 999){
     cat[7]= CleanJet_pt.at(vbs_jet_0);
     cat[9]= CleanJet_eta.at(vbs_jet_0);
     cat[18] = Jet_qgl.at(CleanJet_jetId.at(vbs_jet_0));
     cat[24]= Jet_partonFlavour.at(CleanJet_jetId.at(vbs_jet_0));
}
if (vbs_jet_1 != 999){
     cat[8]= CleanJet_pt.at(vbs_jet_1);
     cat[10]= CleanJet_eta.at(vbs_jet_1);
     cat[19] = Jet_qgl.at(CleanJet_jetId.at(vbs_jet_1));
     cat[25] = Jet_partonFlavour.at(CleanJet_jetId.at(vbs_jet_1));
}
...
if (category==1){
	...
	cat[26]= Jet_partonFlavour.at(CleanJet_jetId.at(v_jet_0));
    }
    if (v_jet_1 != 999){
    cat[12]=CleanJet_pt.at(v_jet_1);
    cat[14]=CleanJet_eta.at(v_jet_1);
    cat[21] = Jet_qgl.at(CleanJet_jetId.at(v_jet_1));
    cat[27] = Jet_partonFlavour.at(CleanJet_jetId.at(v_jet_1));
    }
}
if (category==0) {
