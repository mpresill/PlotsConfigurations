...
'''
    ...
    if opt.doBatch != 0:
      ...
      outputFile=opt.outputDir+'/plots_'+opt.tag+'.root'
      fileList = []
      cleanup = 'cd '+ opt.outputDir+'; '
      allDone=True
      ...
      for iStep in stepList:
        for iTarget in targetList:
          ...
          if not os.path.isfile(opt.outputDir+'/'+iFile) :
      ...
      finalpath = os.path.join(opt.outputDir, finalname)
      ...
      print ' '.join(command)
      if not opt.dryRun:
        subprocess.Popen(command, cwd = os.path.join(opt.outputDir)).communicate()
